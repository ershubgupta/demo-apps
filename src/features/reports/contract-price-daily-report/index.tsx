...
import { contractPriceDailyReportListQueryOptions } from "./api/queries";
import { generateContractPriceDailyReport } from "./api/service";
import { createMobileCard } from "./components/mobile-card";
import { createContractPriceDailyReportColumns } from "./components/table-columns";
import {
...
export { contractPriceDailyReportFilterFields } from "./config/filters";
export { createContractPriceDailyReportColumns } from "./components/table-columns";
export function ContractPriceDailyReportPage() {
  ...
  const params = useMemo<ContractPriceDailyReportListParams>(
    () => ({
  ...
  const query = useQuery(contractPriceDailyReportListQueryOptions(params));
  const data = query.data?.items ?? [];
  const pagination = query.data?.pagination;
  const mutation = useMutation({
    mutationFn: generateContractPriceDailyReport,
    onSuccess: async () => {
      setPollUntil(Date.now() + POLL_TIMEOUT_MS);
      await queryClient.invalidateQueries({
        queryKey: QUERY_KEYS.reports.contractPriceDaily.all,
      });
    },
    onError: (error) =>
      toast.error(t("reports.contractPriceDaily.generationFailed"), {
  ...
  const columns = useMemo(() => createContractPriceDailyReportColumns(t), [t]);
  ...
  return (
    <section className="flex min-h-0 flex-col gap-4 md:h-full">
      <PageHeader
        title={t("reports.contractPriceDaily.title")}
        subtitle={t("reports.contractPriceDaily.subtitle")}
        actions={
          <Button
          ...
          >
            ...
            {t("reports.contractPriceDaily.dailyReport")}
          </Button>
        }
      />
      ...
      {query.isError ? (
        <AppErrorState
          ...
          title={t("reports.contractPriceDaily.loadFailed")}
        />
      ) : (
        <DataTable
          ...
          columns={columns}
          data={data}
          emptyMessage={t("reports.contractPriceDaily.empty")}
